...
import { vendaService, type Venda, type VendaRequest } from '../services/vendaService';
...
export default function Vendas() {
  ...
  const carregar = async () => {
    try { const r = await vendaService.listar(); setLista(r.data); }
  ...
  const abrirVer = (venda: Venda, ver = false) => {
    carregarOpcoes();
    setForm({
  ...
  const salvar = async () => {
    ...
    try {
      editId
        ? await vendaService.atualizar(editId, dto)
        : await vendaService.criar(dto);
  ...
  const handleValidar = async (id: number) => {
    if (!confirm('Validar esta venda?')) return;
    try { await vendaService.validar(id); carregar(); }
    catch (e: any) { alert(e?.response?.data?.mensagem || e?.response?.data?.message || 'Erro ao validar.'); }
  };

  const handleGerarNfe = async (id: number) => {
    if (!confirm('Gerar NF-e para esta venda?')) return;
    try { await vendaService.gerarNfe(id); carregar(); }
  ...
  const handleCancelar = async (id: number) => {
    ...
    try { await vendaService.cancelar(id); carregar(); }
  ...
  return (
    <div style={{ padding: 32, width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <div>
          <h2 style={pageTitle}>Vendas</h2>
          <p style={pageSubtitle}>Gerencie as vendas do salão</p>
        </div>
        <button onClick={abrirNovo} style={btnNew}>+ Nova Venda</button>
      </div>
      ...
      <div style={{ ...card, overflow: 'auto' }}>
        <table className="salon-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#FDF0E8' }}>
              {['Nº Venda', 'Data', 'Cliente', 'Valor Total (R$)', 'Status'].map(h => (
          ...
          <tbody>
            {lista.map(venda => {
              const sc = statusCfg[venda.status] ?? statusCfg.RASCUNHO;
              return (
                <tr key={venda.id} style={{ borderTop: '1px solid #F0E6DC' }}>
                  <td style={{ ...td, fontWeight: 600 }}>{venda.numeroVenda}</td>
                  <td style={td}>{venda.dataVenda}</td>
                  <td style={td}>{venda.cliente?.nome || '—'}</td>
                  <td style={{ ...td, color: '#C97B6B', fontWeight: 600 }}>R$ {Number(venda.valorTotal).toFixed(2)}</td>
                  <td style={td}>
                  ...
                  <td style={td}>
                    {venda.status === 'RASCUNHO' && (<>
                      <button style={{ ...btnEdit, marginRight: 4 }} onClick={() => abrirVer(venda)}>Editar</button>
                      <button style={aBtn('#D1ECF1', '#0C5460')} onClick={() => handleValidar(venda.id)}>Validar</button>
                      <button style={aBtn('#F8D7DA', '#721C24')} onClick={() => handleCancelar(venda.id)}>Cancelar</button>
                    </>)}
                    {venda.status === 'VALIDADA' && (<>
                      <button style={aBtn('#FDF0E8', '#8B6E63')} onClick={() => abrirVer(venda, true)}>Ver</button>
                      <button style={aBtn('#D4EDDA', '#2D6A4F')} onClick={() => handleGerarNfe(venda.id)}>Gerar NF-e</button>
                      <button style={aBtn('#F8D7DA', '#721C24')} onClick={() => handleCancelar(venda.id)}>Cancelar</button>
                    </>)}
                    {venda.status === 'NFE_GERADA' && (<>
                      <button style={aBtn('#FDF0E8', '#8B6E63')} onClick={() => abrirVer(venda, true)}>Ver</button>
                      <button style={aBtn('#D4EDDA', '#2D6A4F')} onClick={() => navigate('/notas-fiscais-saida')}>Ver NF-e</button>
                    </>)}
        ...
        {lista.length === 0 && (
          <div style={{ padding: 48, textAlign: 'center', color: '#8B6E63' }}>
            <p style={{ fontFamily: 'Playfair Display, serif', fontSize: 18 }}>Nenhuma venda cadastrada</p>
          </div>
        )}
      </div>
          ...
          <div style={{ ...modalBox, maxWidth: 900, width: '94%', maxHeight: '92vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 22, color: '#3D2B1F', margin: 0 }}>
                {modoVer ? 'Detalhes da Venda' : (editId ? 'Editar Venda' : 'Nova Venda')}
              </h3>
            ...
            <div style={g12}>
              <div style={{ gridColumn: 'span 4' }}>
                ...
                <input style={inputStyle} placeholder="Ex: V-2024-001" value={form.numeroVenda} onChange={e => setForm({ ...form, numeroVenda: e.target.value })} disabled={modoVer} />
              </div>
              <div style={{ gridColumn: 'span 4' }}>
            ...
            <p style={{ ...sectionTitle, marginTop: 4 }}>Itens da Venda</p>
