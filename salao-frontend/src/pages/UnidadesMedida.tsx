...
import type { UnidadeMedida, UnidadeMedidaRequest } from '../services/unidadeMedidaService';
...
const EMPTY: UnidadeMedidaRequest = { unidadeMedida: '', sigla: '', ativo: true };

export default function UnidadesMedida() {
  const [form, setForm] = useState<UnidadeMedidaRequest>(EMPTY);
  const [editId, setEditId] = useState<number | null>(null);
  const [lista, setLista] = useState<UnidadeMedida[]>([]);
  ...
  const carregar = async () => {
    try { const r = await unidadeMedidaService.listar(); setLista(r.data); }
    catch { setLista([]); }
  };
  ...
  const editar = (item: UnidadeMedida) => {
    setEditId(item.id);
    setForm({ unidadeMedida: item.unidadeMedida, sigla: item.sigla, ativo: item.ativo });
    setErro('');
    formRef.current?.scrollIntoView({ behavior: 'smooth' });
  };

  const salvar = async () => {
    if (!form.unidadeMedida.trim()) { setErro('Nome da unidade é obrigatório.'); return; }
    if (!form.sigla.trim()) { setErro('Sigla é obrigatória.'); return; }
    setErro('');
    try {
      if (editId) await unidadeMedidaService.atualizar(editId, form);
      else await unidadeMedidaService.criar(form);
      setEditId(null);
      setForm(EMPTY);
      await carregar();
    } catch (e: any) {
  ...
  const deletar = async (id: number) => {
    ...
    try { await unidadeMedidaService.deletar(id); carregar(); }
    catch (e: any) { alert(e?.response?.data?.mensagem || e?.response?.data?.message || 'Erro ao excluir.'); }
  };

  const filtrados = lista.filter(item =>
    item.unidadeMedida.toLowerCase().includes(termoBusca.toLowerCase()) ||
    item.sigla.toLowerCase().includes(termoBusca.toLowerCase())
  );

  return (
    <div style={{ padding: 32, width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
      <div style={{ marginBottom: 32 }}>
        <h2 style={pageTitle}>Unidades de Medida</h2>
        <p style={pageSubtitle}>Cadastre as unidades de medida dos produtos</p>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={novoItem} style={btnNew}>+ Nova Unidade</button>
        </div>
      </div>
      ...
      <div style={{ ...card, overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#FDF0E8' }}>
              {['Unidade de Medida', 'Sigla', 'Ativo', 'Ações'].map(h => <th key={h} style={th}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {filtrados.map(item => (
              <tr key={item.id} style={{ borderTop: '1px solid #F0E6DC' }}>
                <td style={{ ...td, fontWeight: 500 }}>{item.unidadeMedida}</td>
                <td style={{ ...td, fontFamily: 'monospace', fontSize: 13 }}>{item.sigla}</td>
        ...
        {filtrados.length === 0 && (
          <div style={{ padding: 48, textAlign: 'center', color: '#8B6E63' }}>
            <p style={{ fontFamily: 'Playfair Display, serif', fontSize: 18 }}>Nenhuma unidade de medida cadastrada</p>
          </div>
        )}
      </div>
    </div>
