...
import { notaFiscalEntradaService, type NotaFiscalEntrada, type TransporteRequest } from '../services/notaFiscalEntradaService';
...
export default function NotasFiscaisEntrada() {
  const [lista,  setLista]  = useState<NotaFiscalEntrada[]>([]);
  ...
  const carregar = async () => {
    try { const r = await notaFiscalEntradaService.listar(); setLista(r.data); }
    catch (e) { console.error('Erro ao carregar notas fiscais de entrada:', e); }
  };

  const abrirEditar = (nf: NotaFiscalEntrada) => {
  ...
  const salvar = async () => {
    if (!editId) return;
    try {
      await notaFiscalEntradaService.atualizarTransporte(editId, form);
  ...
  return (
    <div style={{ padding: 32, width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
      <div style={{ marginBottom: 32 }}>
        <h2 style={pageTitle}>Notas Fiscais de Entrada</h2>
        <p style={pageSubtitle}>Notas geradas automaticamente a partir das compras validadas</p>
      </div>
      ...
      <div style={{ ...card, overflow: 'auto' }}>
        <table className="salon-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#FDF0E8' }}>
              {['Nº Nota', 'Série', 'Data Emissão', 'Fornecedor', 'Valor Total (R$)', 'Transportadora', 'Placa'].map(h => (
          ...
          <tbody>
            {lista.map(nf => (
              <tr key={nf.id} style={{ borderTop: '1px solid #F0E6DC' }}>
                ...
                <td style={td}>{nf.fornecedor?.fornecedor || '—'}</td>
        ...
        {lista.length === 0 && (
          <div style={{ padding: 48, textAlign: 'center', color: '#8B6E63' }}>
            <p style={{ fontFamily: 'Playfair Display, serif', fontSize: 18 }}>Nenhuma nota fiscal de entrada cadastrada</p>
            <p style={{ fontSize: 13, color: '#8B6E63', marginTop: 8 }}>Gere NF-e a partir de uma compra validada em <strong>Compras → Gerar NF-e</strong></p>
          </div>
        )}
      </div>
